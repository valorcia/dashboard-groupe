...
// Lit la config via getConfigValue (KV chiffré → fallback env vars).

import { getConfigValue } from './connectors';

async function odooConfig() {
  return {
    url: await getConfigValue('ODOO_URL'),
    db: await getConfigValue('ODOO_DB'),
    apiKey: await getConfigValue('ODOO_API_KEY'),
    username: (await getConfigValue('ODOO_USERNAME')) || 'admin',
  };
}
...
async function call(endpoint, method, params) {
  const cfg = await odooConfig();
  if (!cfg.url) throw new Error('ODOO_URL is not configured');
...
let cachedUid = null;
let cachedUidCfg = null;

async function authenticate() {
  const cfg = await odooConfig();
  const cfgKey = `${cfg.url}|${cfg.db}|${cfg.username}`;
  if (cachedUid && cachedUidCfg === cfgKey) return cachedUid;
  if (!cfg.db || !cfg.apiKey) {
